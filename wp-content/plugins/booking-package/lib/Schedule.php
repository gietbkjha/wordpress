            ...
            for ($i = 0; $i < $_POST['timeCount']; $i++) {
				...
				**/
				...
				if (is_array($row)) {
					...
					if ($schedule['delete'] == 'true' || isset($schedules[intval($schedule['hour']) . intval($schedule['min'])])) {
				...
				**/
				...
				foreach ((array) $emailData as $emailDataKey => $emailDataValue) {
					...
					**/
					...
					$emailDataValue = str_replace('[bookingDateAndTime]', $date['date'] . $date['time'], $emailDataValue);
					...
					**/
					$emailDataValue = str_replace('[content]', $content, $emailDataValue);
					/**
					for($i = 0; $i < count($form); $i++){
